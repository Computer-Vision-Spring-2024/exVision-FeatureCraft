...
def represent_keypoints(keypoints, DoG):
    ...
    for octave_ind, kp_per_octave in enumerate(keypoints):
        ...
        for dog_idx in range(1, len(DoG) - 1):  # the boundaries are not included
            ...
            )  # create bool 2d array of same size as DoG image
...
def dog_keypoints_orientations(img_gaussians, keypoints, sigma_base, num_bins=36, s=2):
    ...
    for octave_idx in range(len(img_gaussians)):  # iterate over the ocataves
        ...
        for idx, scale_keypoints in enumerate(octave_keypoints):
            ...
            sigma = (
                1.5 * sigma_base * (2**octave_idx) * ((2 ** (1 / s)) ** (scale_idx))
...
def extract_sift_descriptors(img_gaussians, keypoints, base_sigma, num_bins=8, s=2):
    ...
    for i, j, oct_idx, scale_idx, orientation in keypoints:
        ...
        if "index" not in data or data["index"] != (oct_idx, scale_idx):
            ...
            sigma = (
                1.5 * base_sigma * (2**oct_idx) * ((2 ** (1 / s)) ** (scale_idx))
        ...
        )  # clip to remove non-linear illumnation effect (0.2) as descripted by author
...
def apply_sift(
    ...
    img_kp, img_des = computeKeypointsAndDescriptors(
        main_image, n_octaves, s_value, sigma_base, contrast_th, r_ratio
    )
    template_kp, template_des = computeKeypointsAndDescriptors(
        template, n_octaves, s_value, sigma_base, contrast_th, r_ratio
    )

    img_match = match(
        main_image, img_kp, img_des, template, template_kp, template_des, tuning_factor
    )
    return img_match


start_time = time.time()
img_match = apply_sift(
    main_image,
    template,
    n_octaves,
    s_value,
